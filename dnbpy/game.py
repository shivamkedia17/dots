...
class Game:
    def __init__(self, board_size, players, board_state=None):
        ...
        self._board_state = (
            [s for s in board_state]
            if board_state is not None
            else init_board_state(board_size)
        )
    ...
    def select_edge(self, edge_index, player):
        ...
        if player != self._players[self._current_player]:
            raise Exception(
                "next player to play is: %s" % self._players[self._current_player]
            )
        self._board_state[edge_index] = 1
        boxes_made = 0
        for box in self._boxes:
            if (
                box.contains(edge_index)
                and box.is_complete(self._board_state)
                and box not in self._players_to_boxes[player]
            ):
    ...
    def get_next_player(self):
        return [p for p in self._players if p != self._current_player][0]
